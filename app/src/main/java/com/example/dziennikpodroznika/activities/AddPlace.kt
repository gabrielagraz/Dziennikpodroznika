...
import com.example.dziennikpodroznika.databinding.ActivityAddPlaceBinding
...
class AddPlace : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(intent.hasExtra(Constants.EXTRA_PLACE_DETAILS)){
            mPlaceDetails = intent.getParcelableExtra(Constants.EXTRA_PLACE_DETAILS) as PlaceModel?
        }
        ...
        if(mPlaceDetails != null){
            ...
            binding?.ivImageSrc?.setImageURI(Uri.parse(mPlaceDetails!!.image))
        ...
        if(!Places.isInitialized()){
            Places.initialize(this@AddPlace, resources.getString(R.string.google_maps_key))
        }
    ...
    override fun onClick(p0: View?) {
        when(p0!!.id){
            R.id.et_date -> {
                DatePickerDialog(this@AddPlace,
                    dateSetListener,
            ...
            R.id.tv_addimage -> {
                ...
                pictureDialog.setTitle("Wybierz")
            ...
            R.id.btn_save ->{
                if (binding?.etTitle?.text.isNullOrEmpty() || saveImageToInternalStorage == null) {
                    Toast.makeText(this, "Uzupelnij dane o miejscu", Toast.LENGTH_SHORT).show()
                } else {
                    ...
                    if (mPlaceDetails == null) {
                        val handlerResult = handler.addPlace(model)
                        if(handlerResult > 0){
                            Toast.makeText(this, "Miejsce dodane!", Toast.LENGTH_SHORT).show()
                            setResult(Activity.RESULT_OK)
                            finish()
                        }
                    } else {
                        val handlerResult = handler.updatePlace(model)
                        if(handlerResult > 0){
            ...
            R.id.tv_select_current_location ->{
                ...
                if(!isLocationEnabled()){
                    Toast.makeText(this, "Twoj lokalizator jest wyłączony, proszę włacz go w ustawieniach", Toast.LENGTH_SHORT).show()
                    ...
                    Dexter.withActivity(this).withPermissions(
                        ...
                        override fun onPermissionsChecked(report: MultiplePermissionsReport?) {
                            ...
                            if (report.isAnyPermissionPermanentlyDenied) {
                                Toast.makeText(this@AddPlace, "Nie zaakceptowałes ustawień lokalizacji", Toast.LENGTH_SHORT).show()
                            }
                        }
                ...
                Constants.GALLERY ->{
                    if(data != null){
                        ...
                        try {
                            ...
                            Log.e("Zapisane zdjęcie", "path: $saveImageToInternalStorage")

                            binding?.ivImageSrc?.setImageBitmap(selectedImageBitmap)

                        } catch (e: IOException){
                            e.printStackTrace()
                            Toast.makeText(this, "Błąd ładowania zdjęć z galerii", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
        ...
        Dexter.withActivity(this).withPermissions(
            ...
            override fun onPermissionsChecked(report: MultiplePermissionsReport?) {
                if (report?.areAllPermissionsGranted() == true){
                    ...
                    Toast.makeText(this@AddPlace,"Permissions to READ/WRITE storage granted.", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onPermissionRationaleShouldBeShown(
                permissions: MutableList<PermissionRequest>?,
    ...
    private fun showRationalDialogPermissions() {
        AlertDialog.Builder(this).setMessage(
            "Nie wybrano uprawnien. Uprawnienia mozna zmienic w ustawieniach aplikacji")
